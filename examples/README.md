...
| [`vlm/`](vlm/) | `python -m unirl.train_vlm` | `vlm/qwen_vl_argrpo_geo3k_mc_4x8` | `qwen_vl` |
| [`llm/`](llm/) | `python -m unirl.train_vlm` | `llm/qwen3_ar_drpo_4b_base_dpao_sglang` | `qwen3` (shares the AR entrypoint with `vlm/`) |
...
- **`vlm/`** appends dataset + task: `qwen_vl_argrpo_geo3k_mc_4x8` (`geo3k` · multiple-choice).
- AR domains (`vlm/`, `llm/`) append the cluster shape `<N>x<G>` (nodes × GPUs): `..._4x8`.
