...
![SPO overview: the prompt → rollout (old/behavior policy) → group advantage Â → new-policy ratio r = π/π_old → SPO loss pipeline, and the centerpiece contrast between PPO's clipped surrogate (rises then a flat dead shelf past 1+ε) and SPO's smooth quadratic objective (rises to the same peak at 1+ε then turns and curves back down, so the gradient keeps pulling the ratio back).](assets/overview.png)

The figure foregrounds the **bundled paper, SPO** ([*Simple Policy Optimization*](#external-references)) — the ancestor of the trust-region lineage **PPO → GRPO → SPO → DPPO → DRPO** that this folder's code sits downstream of. SPO's one idea: keep PPO's `r·Â` term but **replace the hard `clip`/`min` with a single smooth quadratic penalty** `−|Â|/(2ε)·(r−1)²`, whose per-sample objective (centerpiece, for `Â>0`) peaks exactly at `r=1+ε` — the same point where PPO's surrogate goes flat — so out-of-band ratios still feel a *restoring* gradient instead of PPO's dead, clipped zero. The top strip is the shared rollout → advantage → ratio → loss pipeline; the one **trained** step (new-policy ratio → loss) is the only place SPO differs from PPO. Note the code in *this* folder runs the **DPPO** hard-mask member of the same lineage (see the ⚠️ box above and [The math](#the-math)); SPO is the smooth-penalty ancestor that motivates measuring the trust region by an absolute divergence rather than a raw ratio window.
