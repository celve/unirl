...
![DRPO overview: the prompt → rollout (behavior policy µ) → group advantage Â → replay for new logp π → ratio r and TV shift |π−µ| → reweighted REINFORCE pipeline (single update), and the centerpiece contrast between DPPO's hard-mask step and DRPO's smooth, bounded weight that ramps down and crosses zero into a corrective region at the threshold δ.](assets/overview.png)

The figure's top strip is the pipeline this tutorial runs (`ARSPODPPO.compute_loss_and_backward`): a **prompt** is rolled out by the behavior policy `µ` (SGLang), scored into a per-prompt **group advantage** `Â`, then the trainable policy is **replayed at the sampling temperature** for new log-probs `π` — yielding the ratio `r` and the **Binary-TV shift** `|π−µ|` that drive the reweighted REINFORCE loss, looped once per batch (single update). Its centerpiece is the load-bearing distinction: **DPPO's hard mask** — what the code here actually runs (full gradient, then a cliff to `0` at `δ`) — versus the paper's **DRPO smooth weight** (the formula in [The math](#the-math) — attenuating to `0` at `δ` and *reversing* into a corrective signal beyond it), with the "absolute TV shift, not ratio" choice keeping the weight bounded even on the rare-token tail where SPO's ratio weight blows up.
