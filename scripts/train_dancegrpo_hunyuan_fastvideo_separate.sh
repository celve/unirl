...
#      FIX: Update FastVideoInferenceEngine.get_capabilities() to supports_logprob=True
...
#   Inference: FASTVIDEO_NUM_GPUS GPUs per actor × INFERENCE_ACTORS actors
#   Training:  TRAINING_GPUS GPUs with FSDP
#   Total:     INFERENCE_ACTORS × FASTVIDEO_NUM_GPUS + TRAINING_GPUS
#
...
#   FASTVIDEO_NUM_GPUS=4 INFERENCE_ACTORS=2 TRAINING_GPUS=8 \
#     bash train_dancegrpo_hunyuan_fastvideo_separate.sh
#
# =============================================================================
...
# Actor layout
INFERENCE_ACTORS=${INFERENCE_ACTORS:-2}
...
# Inference layout: total inference GPUs = INFERENCE_ACTORS × FASTVIDEO_NUM_GPUS
INFERENCE_TOTAL_GPUS=$((INFERENCE_ACTORS * FASTVIDEO_NUM_GPUS))
TOTAL_GPUS=$((INFERENCE_TOTAL_GPUS + TRAINING_GPUS))

# Sampling geometry (DanceGRPO-aligned)
NUM_SAMPLES_PER_PROMPT=${NUM_SAMPLES_PER_PROMPT:-8}
PROMPTS_PER_BATCH=${PROMPTS_PER_BATCH:-${INFERENCE_ACTORS}}
...
echo " Inference actors:       ${INFERENCE_ACTORS}"
echo " Inference total GPUs:   ${INFERENCE_TOTAL_GPUS}"
...
echo " Replay old log_prob:    ${FASTVIDEO_REPLAY_LOG_PROBS} (experimental)"
echo "======================================================"
...
python -m diffusionrl.train \
    ...
    --fastvideo-replay-log-probs ${FASTVIDEO_REPLAY_LOG_PROBS} \
    \
    `# ===== FastVideo Separate Mode Layout =====` \
    --colocate-inference-training false \
    --inference-num-gpus-per-node ${INFERENCE_TOTAL_GPUS} \
