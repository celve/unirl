...
# NOTE: colocate_inference_training currently supports only single-GPU inference actors.
...
if [ "${FASTVIDEO_NUM_GPUS}" -gt 1 ]; then
    echo "ERROR: colocate_inference_training=true does not support multi-GPU inference actors."
...
python -m diffusionrl.train \
    ...
    --inference-num-gpus-per-node ${GPUS_PER_NODE} \
