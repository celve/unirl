...
#
# MODE: Separate — inference actors and training actors on DIFFERENT GPUs
#   - Inference actors: run HunyuanSampler (SDE sampling + log_prob)
#   - Training actors: FSDP-wrapped transformer for GRPO loss backprop
#   - Weight sync: training → inference after each rollout
...
#   - FSDPInferenceEngine + FSDPHunyuanSampler: supports_logprob=True  ✅
#   - FastVideoInferenceEngine:                 supports_logprob=False ✗
...
#   INFERENCE_GPUS=4 TRAINING_GPUS=4 bash train_dancegrpo_hunyuan_separate.sh
#
# =============================================================================
...
# Sampling geometry
# DanceGRPO: num_generations=24 per rank. In separate mode, each inference
# actor generates NUM_SAMPLES_PER_PROMPT samples per prompt.
NUM_SAMPLES_PER_PROMPT=${NUM_SAMPLES_PER_PROMPT:-8}
PROMPTS_PER_BATCH=${PROMPTS_PER_BATCH:-${INFERENCE_GPUS}}
