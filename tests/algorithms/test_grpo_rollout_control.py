"""Unit tests for ``diffusionrl.algorithms.rollout_control``.

Locks the driver-side contract that ``rollout/pipeline.py`` depends on:
...
pytest.importorskip(
    "diffusionrl.algorithms.rollout_control",
    reason="Pre-existing circular import in algorithms package init "
    "(types/conditions/base ↔ distributed ↔ rollout/engine).",
    exc_type=ImportError,
)

from diffusionrl.algorithms.rollout_control import (  # noqa: E402
    GRPORolloutControl,
...
def test_attributes_exposed_for_train_driver() -> None:
    """``train.py:198`` reads ``getattr(ctrl, 'samples_per_prompt', 1)``
