...
(``train.py`` via ``compute_initial_noise_for_request`` + the trainside
engine path) puts per-sample x_T into ``request_conditions['initial_latents']``,
but pre-Batch-5 the pipelines all called their internal ``generate_latents``
...
We test SD3 here as the canonical pattern — the other 4 pipelines
...
from diffusionrl.models.sd3.diffusion import (
    SD3DiffusionStage,
    SD3DiffusionStep,
)
from diffusionrl.models.sd3.pipeline import SD3Pipeline
from diffusionrl.models.sd3.text_embed import SD3TextEmbedStage
