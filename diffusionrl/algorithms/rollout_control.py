...
The trainer-side :class:`StageAlgorithm` (``algorithms.DiffusionGRPO`` /
...
from diffusionrl.algorithms.normalizers import normalize_global, normalize_grouped
...
@register_config(
    group="algorithm",
    name="grpo",
    target="diffusionrl.algorithms.rollout_control.GRPORolloutControl",
    mutable=True,
)
@dataclass
...
class GRPORolloutControl:
    ...
    # ``ray/mixins/rollout_pipeline.py`` (compute_advantages,
...
@register_config(
    group="algorithm",
    name="nft",
    target="diffusionrl.algorithms.rollout_control.NFTRolloutControl",
    mutable=True,
)
@dataclass
