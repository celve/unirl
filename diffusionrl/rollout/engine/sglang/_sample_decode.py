...
:mod:`diffusionrl.rollout.engine.sglang.response`.
"""
...
def _tensorize(value: Any) -> Optional[torch.Tensor]:
    ...
    of the same. Returns ``None`` when no sensible conversion exists,
    instead of raising — callers decide whether absence is an error.
    """
    if value is None:
        return None
