...
concatenate per-row into the diffusionrl varlen ``TextSegment``.
...
def extract_ar_segment(per_request_outputs: Sequence[Sequence[Any]]) -> Optional[Any]:
    ...
    ``sample_indices`` is per-sample ``arange(N)`` and ``positions`` is
    ``zeros(N)`` (one AR segment per sample, each at position 0 within
    its sample's segment sequence).
