...
Composes five phases — load_prompts → plan_requests → exec_request →
aggregate → convert_training_data. Driven by a
...
if TYPE_CHECKING:
    from diffusionrl.algorithms.rollout_control import GRPORolloutControl
...
def _reject_unsupported_media_refs(batch: Dict[str, Any], *, context: str) -> None:
    ...
    The driver now consumes the ``(image, condition)`` (modality,
...
class RolloutPipeline:
    ...
    def load_prompts(
        ...
        *,
        ...
        _reject_unsupported_media_refs(prompt_batch, context="RolloutPipeline.load_prompts")
    ...
    def convert_training_data(
        ...
        *,
        ...
        if text_prim is None or not getattr(text_prim, "texts", None):
            raise RuntimeError("RolloutPipeline.convert_training_data: req.primitives['text'] missing.")
        ...
        if len(request_sids) != len(request_texts):
            raise RuntimeError(
                f"RolloutPipeline.convert_training_data: request sample_ids "
        ...
        try:
            ...
            raise RuntimeError(
                f"RolloutPipeline.convert_training_data: response sample_id {missing!r} not present in request."
            ) from None
    ...
    def run_eval(
        ...
        *,
        ...
        _reject_unsupported_media_refs(request_batch, context="RolloutPipeline.run_eval")
