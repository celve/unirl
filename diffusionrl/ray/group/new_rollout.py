"""New rollout actor group: spawn + dispatch + control plane for the
``RolloutReq`` / ``RolloutResp`` path.

Sibling of :class:`diffusionrl.ray.group.rollout.RolloutActorGroup` (legacy
types). Spawns :class:`diffusionrl.ray.new_rollout_actor.NewRolloutActor`
handles and exposes the same control-plane surface (``sleep`` /
``wake_up`` / weight-sync group setup). The data plane drives sharding via
:meth:`RolloutPlan.shard_grouped_req` rather than the legacy
``shard`` / ``shard_grouped``.

Construction is single-step: ``NewRolloutActorGroup(cfg=cfg,
placement=placement)`` validates the engine class, spawns Ray handles, syncs
on ``health_check``, and wires up the rollout plan in one call. The legacy
``bootstrap`` classmethod factory is intentionally absent here — the new
group is symmetric with :class:`NewRolloutActor` (whose ``__init__`` already
does heavy work).
"""
...
from diffusionrl.rollout.engine.base import BaseRolloutEngine as NewBaseRolloutEngine
...
def _spawn_actors(
    *,
    ...
    """Spawn ``NewRolloutActor`` Ray handles per ``placement.rollout_actors``.

    Module-level helper (legacy parallel: ``RolloutActorGroup._spawn_actors``).
    ...
    """
    from diffusionrl.ray.new_rollout_actor import NewRolloutActor
    ...
    # Same determinism env vars as the legacy group: cuBLAS workspace must
    # land in os.environ before torch import.
    ...
    logger.info(
        "Creating %d new rollout actors (noset=%s, colocate=%s, stride=%d, ray_num_gpus=%s)",
        len(actors),
        noset,
        colocate,
        stride,
    ...
    for actor in actors:
        ...
        handle = NewRolloutActor.options(
...
class NewRolloutActorGroup(ActorGroup):
    ...
    Single-step construction: ``NewRolloutActorGroup(cfg=cfg, placement=placement)``
    validates that ``cfg.rollout.engine._target_`` resolves to a subclass of the
    new ABC, spawns ``NewRolloutActor`` handles, syncs on ``health_check``, and
    materializes the rollout plan — in one call.
    """
    ...
    def __init__(
        ...
        *,
        ...
        """Spawn ``NewRolloutActor`` handles directly from a Hydra ``DictConfig``.
        ...
        the new-ABC :class:`BaseRolloutEngine`; otherwise the legacy
        :class:`RolloutActorGroup` should be used instead.
        """
        import hydra
        ...
        if not (isinstance(engine_cls, type) and issubclass(engine_cls, NewBaseRolloutEngine)):
            raise ValueError(
                f"NewRolloutActorGroup requires the engine at {engine_target!r} to subclass "
                f"diffusionrl.rollout.engine.base.BaseRolloutEngine (the new ABC). "
                f"Use diffusionrl.ray.group.rollout.RolloutActorGroup for legacy "
                f"engines (SGLang / FSDP)."
            )
        sampler_engine_type = (
        ...
        logger.info("New rollout actors initialized: %d handles", len(actors))
        ...
        logger.info("NewRolloutActorGroup ready (engine=%s)", sampler_engine_type)
    ...
    def from_train_group(
        cls,
        train_group: "NewTrainActorGroup",
        *,
        cfg: DictConfig,
    ) -> "NewRolloutActorGroup":
        ...
        :class:`NewRolloutPipelineMixin` surface (see
        :class:`NewTrainActor` for the host-contract install).

        Health checks are skipped — the train actors already passed their
        own during ``NewTrainActorGroup`` construction; doing them again
        only buys latency.
        """
        from diffusionrl.config.instantiate import materialize
        ...
        if not is_direct_sampling(cfg):
            raise ValueError(
                "NewRolloutActorGroup.from_train_group requires "
        ...
        if not actors:
            raise ValueError("NewRolloutActorGroup.from_train_group: train_group has no actor handles to adopt.")
        ...
        logger.info(
            "NewRolloutActorGroup.from_train_group: adopted %d train actor "
            "handle(s) as rollout actors (engine=trainside)",
            len(actors),
        )
        return instance

    # ------------------------------------------------------------------
    # Data plane: generate (parity with legacy group; production path uses
    # the driver-side NewRolloutPipeline for shard_grouped_req routing)
    # ------------------------------------------------------------------
