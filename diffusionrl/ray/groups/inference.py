"""Inference worker-group implementation."""
...
class InferenceActorGroup(BaseActorGroup):
    ...
    def async_generate(
        ...
        **kwargs,
        ...
        if not isinstance(prompts, list) or len(prompts) == 0:
            raise ValueError(
                "InferenceActorGroup.generate requires non-empty text prompts. "
        ...
        if (
            ...
            logger.warning(
                "InferenceActorGroup(%s) now uses prompt-only generation input; "
    ...
    def update_weights_from_path(
        ...
        if len(checksum_groups) > 1:
            ...
            raise RuntimeError(
                "Checksum mismatch across inference actors after update_weights_from_path: "
                f"{details}"
            )
        checksum_payload = dict(next(iter(checksum_groups.keys())))
        logger.info(
            "Verified consistent inference checksum across %d actors: %s",
            len(checksum_rows),
            checksum_payload,
        )
