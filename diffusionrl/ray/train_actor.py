"""diffusionrl train actor.

Drives the training stack end-to-end: a :class:`Policy` composed via
:func:`compose_policy` over a :class:`Stage` from a ``models`` pipeline,
trained by :class:`StageTrainStack` against a ``RolloutResp``.

The Policy stack (``LoRAPolicy`` → ``FSDPPolicy`` → ``EMAPolicy``) owns
FSDP wrap, LoRA injection and EMA shadow. The actor reads
``cfg.training.policies`` (an ordered list of policy configs) and
``cfg.training.policy_source`` (the slot name on the pipeline whose
Stage anchors the stack).

Slot-keyed algorithms: ``cfg.algorithms.<slot>`` (plural). Each entry
materializes a :class:`StageAlgorithm`. The actor injects ``stage=``
(and optionally ``params=``) at instantiate time so the algorithm preset
can stay a static ``_target_`` declaration.
...
- **Separate sampling** (default; ``cfg.rollout.engine: vllm_omni``
  or sglang): rollout runs on a sibling :class:`RolloutActor`;
...
- **Direct sampling** (``cfg.rollout.engine: trainside``): the
  ...
  plus the rest of the :class:`RolloutPipelineMixin` host-contract
  (``self._rollout_plan`` / ``self.algorithm`` / ``self._reward_pipeline``
  / ``self.generate(req)``) so :class:`RolloutActorGroup.from_train_group`
...
from diffusionrl.training import StageTrainStack, TrainOptimizerStepResult
from diffusionrl.training.ema_policy import EMAPolicy
from diffusionrl.training.fsdp_policy import FSDPPolicy
from diffusionrl.training.policy import Policy, compose_policy, walk_source_chain
...
def _detect_lora_on_model(model: Any) -> bool:
    ...
    ``training/fsdp_policy.py:_extract_peft_lora_state`` checks: the
...
def _materialize_policy_config(node: Any) -> Any:
    ...
    if not target_path:
        raise ValueError(
            ...
            "use a @register_config preset (group='training/policy')."
        )
...
class TrainActor(
    ...
    """Train actor for the Policy-stack path.
    ...
    :class:`RolloutPipelineMixin` contributes the
    ``generate_buffered`` / ``attach_reward`` / ``run_rollout_pipeline``
    surface used by :class:`RolloutActorGroup.from_train_group` in
    ...
    def __init__(
        ...
        *,
        ...
        # used in ``training/fsdp_policy.py:_extract_peft_lora_state``.
        ...
        for slot, alg_node in cfg.algorithms.items():
            ...
            if target.endswith(".DiffusionNFT"):
                from diffusionrl.training.nft_lora_policy import NFTLoRAPolicy
                ...
                if nft_lora is None:
                    raise RuntimeError(
                        f"TrainActor: algorithms.{slot}._target_={target!r} "
        ...
        logger.info(
            "Rank %s: TrainActor initialized (slots=%s, policy_chain=%s, direct_sampling=%s)",
            self.rank,
    ...
    def generate(self, req: RolloutReq) -> RolloutResp:
        ...
        Mirror of :meth:`RolloutActor.generate` for the direct-sampling
        ...
        if self.engine is None:
            raise RuntimeError(
                "TrainActor.generate: cfg.rollout.engine must target "
                "TrainsideRolloutEngine for the train actor to serve rollouts. "
                "In separate-sampling mode, route generation through "
                "RolloutActor instead."
            )
        if int(req.batch_size) == 0:
            raise ValueError("TrainActor.generate requires non-empty req (batch_size>0).")
    ...
    def _ensure_reward_pipeline(self) -> RewardPipeline:
        ...
        if self._reward_pipeline is None:
            if self._reward_config is None:
                raise RuntimeError(
                    "TrainActor._ensure_reward_pipeline: cfg.reward was not "
