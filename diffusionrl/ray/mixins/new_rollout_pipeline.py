...
class _RolloutRespMeta(Batched):
    """Per-handle key for new-path buffered RolloutResp shards.
...
def _build_legacy_response_view(
    ...
    *,
    ...
    if text_prim is None or not getattr(text_prim, "texts", None):
        raise RuntimeError("NewRolloutPipelineMixin: req.primitives['text'] must be non-empty for reward scoring.")
    texts = list(text_prim.texts)
    sids = list(req_shard.sample_ids)
    gids = list(req_shard.group_ids)
    if len(texts) != len(sids):
        raise RuntimeError(
            f"NewRolloutPipelineMixin: text count {len(texts)} != sample_ids count {len(sids)}; "
...
class NewRolloutPipelineMixin:
    ...
    def generate_buffered(self, req: "RolloutReq") -> List["BufferHandle"]:
        ...
        if len(req_shards) != len(resp_shards):
            raise RuntimeError(
                f"NewRolloutPipelineMixin.generate_buffered: req split has "
    ...
    def attach_reward(self, handle: "BufferHandle") -> None:
        ...
        try:
            ...
            raise RuntimeError(
                f"NewRolloutPipelineMixin.attach_reward: handle {handle.id} has "
...
__all__ = ["NewRolloutPipelineMixin"]
