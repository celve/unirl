"""diffusionrl rollout actor for the new ``RolloutReq`` / ``RolloutResp`` path.

Sibling of :class:`diffusionrl.ray.rollout_actor.RolloutActor`. Wraps a new-ABC
:class:`diffusionrl.rollout.engine.base.BaseRolloutEngine` (currently
``VLLMOmniRolloutEngine`` for HunyuanImage-3) and speaks
``RolloutReq``/``RolloutResp`` end-to-end. The legacy actor is left untouched
and stays the path for SGLang / FSDP engines.

Key differences vs the legacy actor:

- One-shot engine construction. ``build(cfg.rollout.engine, device=..., strategy=...,
  rank=..., model_config=...)`` returns a fully-usable engine; no separate
  ``engine.initialize(device)`` step.
- No post-ctor ``engine.strategy = ...`` mutation. The strategy travels via the
  build kwarg and (for per-call control) via ``RolloutReq.stage_params``.
- ``generate(req)`` returns a ``RolloutResp`` directly — chunking is handled by
  :func:`diffusionrl.rollout.engine.chunked_engine_generate_req` against the
  new types.
"""
...
@ray.remote
class NewRolloutActor(ConfigActor, RolloutWeightSyncMixin, DistributedMixin, NewRolloutPipelineMixin, Buffer):
    ...
    Example::
        ...
        actor = NewRolloutActor.options(num_gpus=8).remote(
    ...
    def _setup_distributed_env(self) -> None:
        """Same env setup as the legacy actor, kept identical so multi-GPU
        rollout actors can be NOSET-pattern Slime-allocated regardless of
        which engine ABC the actor wraps.
        """
    ...
    def generate(self, req: RolloutReq) -> RolloutResp:
        ...
        if int(req.batch_size) == 0:
            raise ValueError("NewRolloutActor.generate requires non-empty req (batch_size>0).")
    ...
    def num_stages(self) -> int:
        ...
        """
        if self.engine is None:
            raise RuntimeError("NewRolloutActor.num_stages: engine not initialized")
    ...
    def tp_per_stage(self) -> dict:
        ...
        """
        if self.engine is None:
            raise RuntimeError("NewRolloutActor.tp_per_stage: engine not initialized")
    ...
    def loaded_param_checksums(
        ...
        *,
        ...
        """
        if self.engine is None:
            raise RuntimeError("NewRolloutActor.loaded_param_checksums: engine not initialized")
    ...
    def list_param_names(
        ...
        *,
        ...
        """
        if self.engine is None:
            raise RuntimeError("NewRolloutActor.list_param_names: engine not initialized")
        omni = getattr(self.engine, "_omni", None)
        if omni is None:
            raise NotImplementedError(
                f"engine {type(self.engine).__name__} does not expose ``_omni`` (vllm-omni only today)."
