...
@register_config(
    ...
    name="fsdp",
    target="diffusionrl.training.fsdp_policy.FSDPPolicy",
)
@dataclass
...
class FSDPPolicy(PolicyBase):
    ...
    def _wrap_model(self) -> None:
        ...
        # the one experimentally validated on HI3 1024x1024 (peak 92 GB
        # OOM -> 45 GB). The ``apply_activation_checkpointing`` path tripped
        # a ``CheckpointError: Recomputed values ... different metadata``
        # (bf16 saved vs fp32 recomputed) — autocast context wasn't preserved
        # across recompute when kwargs were forwarded through
        # ``checkpoint(... **kwargs)``. Closing over kwargs (so
