...
This module is completely self-contained — it does NOT import from wan21.py.
"""
...
@dataclass
class WAN22ModelBundleConfig(ModelBundleConfig):
    """Wan2.2-T2V specific configuration.

    Extends the base ModelBundleConfig with WAN-specific and dual-transformer parameters.
    """
...
class WAN22ModelBundle(ModelBundle):
    ...
    def __init__(self, config: WAN22ModelBundleConfig):
        ...
        # Base ModelBundle.__init__ sets up device, dtype, pretrained_path, etc.
        super().__init__(config)

        # WAN-specific fields (from wan21 init logic)
    ...
    def encode_inputs(
        ...
        **kwargs: Any,
        ...
        if video is not None:
            raise NotImplementedError("WAN22ModelBundle does not support video conditioning yet")
    ...
    def encode_image_inputs(
        ...
        *,
        ...
        # WAN I2V: encode image via CLIP vision for cross-attention
    ...
    # ------------------------------------------------------------------
    # Conditioning image preparation helper
    # ------------------------------------------------------------------
    ...
    def _spatial_patch_size(self) -> int:
        ...
        Reads from _high_noise_transformer directly because self.transformer is
        WanDualTransformer (no .config.patch_size at the composite level).
