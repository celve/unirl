...
Mirrors :class:`diffusionrl.models_new.sd3.config.SD3PipelineConfig` shape —
weights+params only, no LoRA / FSDP / offload knobs. Those lifecycle
concerns live outside the bundle (in ``training_new`` Policy stack).
...
@register_config(
    group="model",
    name="wan21_v2",
    target="diffusionrl.models_new.wan21.WAN21Pipeline.from_config",
    mutable=True,
)
@dataclass
class WAN21PipelineConfig:
    ...
    # UMT5 max sequence length — WAN T5 uses long context (matches legacy
    # ``WAN21ModelBundleConfig.max_sequence_length``).
