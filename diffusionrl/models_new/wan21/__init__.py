...
package provides the equivalent on the new pipeline (via ``train_new.py``
+ ``model: wan21_v2``).
...
calls in ``config.py``), so importing ``diffusionrl.models_new.wan21``
is enough to make ``model/wan21_v2`` resolvable in the ConfigStore.
"""

from diffusionrl.models_new.wan21.bundle import WAN21Bundle
...
from diffusionrl.models_new.wan21.config import WAN21PipelineConfig
from diffusionrl.models_new.wan21.diffusion import (
...
from diffusionrl.models_new.wan21.text_embed import WAN21TextEmbedStage
from diffusionrl.models_new.wan21.vae import WAN21VAEDecodeStage
