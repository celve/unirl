...
- ``embeds`` is the encoder's ``last_hidden_state`` with **padded
  ...
  ``diffusionrl/models/wan21.py:687-690``.)
...
and ``diffusionrl/models/wan21.py:665-697`` (WANTextEncoderWrapper) but
is intentionally re-inlined here: the new-design path does NOT import
...
class _TextEncoderBundle(Protocol):
    ...
    :class:`diffusionrl.models_new.wan21.bundle.WAN21Bundle` and
    :class:`diffusionrl.models_new.wan22.bundle.WAN22Bundle` satisfy
