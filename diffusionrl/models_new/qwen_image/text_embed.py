...
:class:`diffusionrl.models_new.sd3.SD3TextEmbedStage` in three ways:
...
Math mirrors ``models/qwen_image.py::_get_qwen_prompt_embeds`` and
``models/qwen_image.py::encode_prompt`` (PR #104) byte-for-byte at the
spec level. The new-design path does NOT import legacy code; the two
implementations must stay in sync via review / test.
"""
...
# Chat-template constants. Identical to the legacy
# ``QwenImageModelBundle`` constants — copied here so the new-design
# stage stays decoupled from legacy.
PROMPT_TEMPLATE = (
