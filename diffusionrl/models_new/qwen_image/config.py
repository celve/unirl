...
Sibling of :class:`diffusionrl.models_new.sd3.SD3PipelineConfig`. Carries
...
@register_config(
    group="model",
    name="qwen_image_v2",
    target="diffusionrl.models_new.qwen_image.QwenImagePipeline.from_config",
    mutable=True,
)
@dataclass
class QwenImagePipelineConfig:
    ...
    # not per-request shape. Defaults match the legacy QwenImageSampler.
    ...
    # (mirrors legacy ``QwenImageModelBundleConfig.max_sequence_length``).
    ...
    # LoRA hints for rollout-side engines (e.g. ``sglang_new``). Mirrors
