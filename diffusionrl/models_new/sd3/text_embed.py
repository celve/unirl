...
Triple-encoder math mirrors ``samplers/fsdp/sd3_sampler.py::_encode_prompt``
(in‑repo reference implementation) but is intentionally re-inlined here:
the new-design path does NOT import legacy code, so the two encoders must
stay in spec sync via review / test, not via shared helpers.
"""
