...
from diffusionrl.models_new.wan21.diffusion import WAN21DiffusionParams
...
class WAN22DiffusionStage(DiffusionStage[WAN21Conditions]):
    ...
    def diffuse(
        ...
        *,
        ...
        sample_indices = torch.arange(batch_size, dtype=torch.long, device=device)
        positions_tensor = torch.zeros(batch_size, dtype=torch.long, device=device)
        ...
        # as in WAN 2.1 (see ``models_new/wan21/diffusion.py``).
    ...
    def trainable_module(self) -> "torch.nn.Module":
        ...
        **FSDP wrapping policy** (matches
        ``diffusionrl/training_new/fsdp_policy.py``):
        ...
        **LoRA-on-composite assumption** (worth a GPU smoke):
        :class:`diffusionrl.training_new.lora_policy.LoRAPolicy` calls
        ...
        explicitly per sub-transformer to be safe; the new path relies
