...
``train_new.py`` + ``model: wan22_v2``. The legacy ``diffusionrl/models``
...
:class:`diffusionrl.models_new.wan22.bundle.WanDualTransformer` so the
...
in ``config.py``), so importing ``diffusionrl.models_new.wan22`` is
enough to make ``model/wan22_v2`` resolvable in the ConfigStore.
"""

from diffusionrl.models_new.wan22.bundle import WAN22Bundle, WanDualTransformer
from diffusionrl.models_new.wan22.config import (
    DEFAULT_BOUNDARY_RATIO,
    WAN22PipelineConfig,
)
from diffusionrl.models_new.wan22.diffusion import (
