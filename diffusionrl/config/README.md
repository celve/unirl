...
`diffusionrl.config` owns the typed Hydra surface. Config dataclasses are
registered near the code that consumes them, then imported by
`register_all_configs()` before Hydra composes `conf/train.yaml`.
...
`diffusionrl.train` runs cross-component validators before any Ray actor is
