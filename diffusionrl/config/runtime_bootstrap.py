"""Resolved runtime bootstrap config built once on the driver."""
...
@dataclass(frozen=True)
class ResolvedRolloutBootstrap:
    mode: str
    service_engine: str
...
@dataclass(frozen=True)
class ResolvedTrainingBootstrap:
...
class ResolvedRuntimeConfig:
    ...
    rollout: Optional[ResolvedRolloutBootstrap]
    training: ResolvedTrainingBootstrap


def resolve_runtime_placement_spec(
    args: Any,
    *,
    rollout_mode_info: Optional[ResolvedRolloutModeInfo] = None,
) -> ResolvedPlacementSpec:
    resolved_mode_info = rollout_mode_info if rollout_mode_info is not None else derive_rollout_mode_info(args)
...
def resolve_runtime_config(
    ...
    training_plan = derive_training_plan(args)
    placement = resolve_runtime_placement_spec(
        args,
    ...
    rollout: Optional[ResolvedRolloutBootstrap] = None
    if placement.rollout_num_nodes > 0 and placement.rollout_num_gpus_per_node > 0:
        ...
        rollout = ResolvedRolloutBootstrap(
...
__all__ = [
    "ResolvedPlacementSpec",
    "ResolvedRolloutBootstrap",
    "ResolvedRuntimeConfig",
    "ResolvedTrainingBootstrap",
