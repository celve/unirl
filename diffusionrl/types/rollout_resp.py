...
track). For per-segment-row fields (like LatentSegment's latents)::

    track = resp.tracks["image"]
    mask = track.segment.sample_indices == i
    latents_i = track.segment.latents[mask]
...
from typing import Any, Callable, Dict, Iterable, List, Literal, Optional, Sequence, Tuple, Type, TypeVar, Union
...
class RolloutTrack(Transportable):
    ...
    @classmethod
    def concat(cls: Type[TR], items: Sequence[TR]) -> TR:
        """Concat with per-track ``segment.sample_indices`` offset shift.
