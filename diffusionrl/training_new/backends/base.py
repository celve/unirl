"""Schema dataclasses for the new-design training stack.

Sibling of :mod:`diffusionrl.training.backends.base`. Holds only the
config-layer schemas the new training path consumes:
...
The legacy ``TrainBackend`` Protocol, ``TrainBackendConfig`` marker base,
and capability/launch-spec dataclasses are intentionally absent — the
new path uses Policy-stack composition instead of a Backend protocol.
"""
...
@register_config(group="training_new/optimizer", name="default")
@dataclass
...
@register_config(group="training_new/lr_scheduler", name="default")
@dataclass
...
@register_config(group="training_new/topology", name="default")
@dataclass
class TrainTopology:
