"""Authoritative training batch/update plan for the new-design path.

Sibling of :mod:`diffusionrl.training.plan`. Consumed by
:class:`diffusionrl.training_new.stack.StageTrainStack` (which reads
``cfg.training.plan.micro_batch_size``). Authored in cfg via the
``training_new/plan`` preset.
"""
...
@register_config(group="training_new/plan", name="default")
@dataclass
class TrainingPlan:
