...
训练语义放在 `conf/*.yaml`。Shell 脚本只负责准备环境、启动 Ray、设置路径/logging 默认值，并转发 recipe 名和 Hydra overrides。

## 启动器

单机作业使用 single-node launcher；多机作业使用 role-aware multinode launcher，让 worker 加入 Ray head 后再训练。第一个参数是 `conf/` 下的 recipe 名；`ENTRY` 选择非 diffusion 入口（`train_vlm`、`train_pe`、`train_hi3`）。
...
ENTRY=train_vlm bash scripts/run_experiment_multinode_taiji.sh argrpo_qwen_vl_geo3k_mc_sglang_4x8
```
