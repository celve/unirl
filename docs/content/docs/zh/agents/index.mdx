...
- 维护中的训练入口是 `python -m unirl.train_diffusion --config-name=<recipe>`（以及 `train_vlm` / `train_pe` / `train_hi3`）。
- 扁平的 `conf/<recipe>.yaml` 文件是权威配置面。
...
| 选择实验 | `/zh/docs/configuration/experiments`，然后 `conf/<name>.yaml` |
