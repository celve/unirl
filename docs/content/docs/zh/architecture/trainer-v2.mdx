...
| `python -m unirl.train_hi3` | `trainer/hi3.py` | HunyuanImage3（AR + diffusion 混合） |
...
`TrainStack`**，每个 track 一个。HunyuanImage3 的 AR + diffusion 混合训练用 `hi3_stack.py`（多 stage 变体）。
...
[Train Stack README](/en/docs/architecture/readme-reference/unirl-train)。

## 配置形态

`conf/*.yaml` 是扁平目录；每个 recipe 用 `_target_` 实例化各 sibling（没有 Hydra config-group 覆盖）：
