...
Training semantics live in `conf/*.yaml`. Shell scripts only prepare the runtime
...
argument is a recipe name from `conf/`; `ENTRY` selects a non-diffusion
entrypoint (`train_vlm`, `train_pe`, `train_hi3`).
