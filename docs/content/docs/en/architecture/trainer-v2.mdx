...
| `python -m unirl.train_hi3` | `trainer/hi3.py` | HunyuanImage3 (mixed AR + diffusion) |
...
track. HunyuanImage3's mixed AR + diffusion training uses `hi3_stack.py`, a
multi-stage variant.
...
`conf/*.yaml` is a flat directory; each recipe instantiates its siblings by
