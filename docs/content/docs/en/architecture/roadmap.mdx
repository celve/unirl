...
- **Baseline.** The trainer exposes a swappable `backend:` block (see `conf/*.yaml`);
...
|---|---|---|---|
| Policy-Gradient / PPO | rollout log-probs + advantages through a clipped / KL surrogate | GRPO family (FlowGRPO / DanceGRPO / MixGRPO), DPPO, ARGRPO, SPO-DPPO | DDPO, DPOK, KL / reference policy, full PPO (critic + GAE) |
