...
primer, not an API reference; code-adjacent contracts live under
[README Reference](/en/docs/architecture/readme-reference).
...
| Train stack | `TrainStack` — single-stage loss/backward driver; multi-track uses sibling stacks; HI3 uses `hi3_stack`. |
...
- Algorithm loss math and the reward→advantage path: [Algorithms Package README](/en/docs/architecture/readme-reference/unirl-algorithms).
