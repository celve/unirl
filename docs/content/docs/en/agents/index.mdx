...
- Treat `python -m unirl.train_diffusion --config-name=<recipe>` (and `train_vlm` / `train_pe` / `train_hi3`) as the maintained runtime entry.
- Treat the flat `conf/<recipe>.yaml` files as the authoritative configuration surface.
...
| Pick an experiment | `/en/docs/configuration/experiments`, then `conf/<name>.yaml` |
