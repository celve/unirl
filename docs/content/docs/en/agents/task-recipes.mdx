...
- closest `conf/<recipe>.yaml`

Edit:

- `conf/<new_recipe>.yaml`
- this docs page and `/en/docs/configuration/experiments` if the recipe is maintained

Check:

```bash
python -m unirl.train_diffusion --config-name=<new_recipe> --cfg job --resolve
```
...
- `unirl/distributed/weight_sync/README.md`
- `unirl/rollout/README.md`
- the recipe's `sync` selection in `conf/<recipe>.yaml`

Edit:

- `unirl/distributed/weight_sync/`
- `conf/<recipe>.yaml` when selecting or tuning a backend
