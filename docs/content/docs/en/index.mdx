...
`<recipe>` is a self-contained YAML filename (without `.yaml`) in the flat `conf/` directory — for example:
...
Detailed runtime and module contracts live in the README Reference pages under each docs section in the sidebar, sourced from README files next to the code.
