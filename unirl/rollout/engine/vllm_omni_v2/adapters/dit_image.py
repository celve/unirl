...
Two members with very different request sides but one output shape:

- ``sd35_t2i`` — SD3.5-medium single diffusion stage; prompt dicts are the
  ``{"prompt", "negative_prompt"}`` shape ``StableDiffusion3Pipeline.forward``
  accepts; conditions come from the ``encode_prompt`` text capture.
- ``dit_recaption`` — standalone HI3 DiT that eats an externally-injected
  recaption (``req.primitives['cot_text']``); one seeded ``generate()`` call
  per prompt; conditions come from the fused multimodal capture.
"""
...
class DitRecaptionAdapter(DiTImageAdapter):
    ...
    **One call per prompt, seeded here.** Per-image distinct seeds cannot
    ...
    def build_inputs(self, req: RolloutReq) -> List[GenerateCall]:
        ...
        _, sys_type, _ = resolve_task("dit_recaption", req.stage_config or {})
        diff_params = get_diffusion_params(req.sampling_params)
...
__all__ = ["DiTImageAdapter", "DitRecaptionAdapter", "Sd35T2iAdapter"]
