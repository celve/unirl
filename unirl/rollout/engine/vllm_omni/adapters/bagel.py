...
- **σ off-by-one.** BAGEL's ``generate_image`` builds its σ schedule internally
  ...
  worker must receive ``num_inference_steps = T + 1``. The engine pins the
  diffusion params' ``sigmas`` for ``T`` steps (``T + 1`` σ points) via this
  adapter's static-shift :meth:`schedule_policy`; BAGEL's internal schedule then
  equals it (BAGEL hardwires ``timestep_shift = 3.0`` — the trainside shift — and
  the σ formula is identical), and the response-side ``verify_engine_used_sigmas``
...
class BagelInputAdapter(DitInputAdapter):
    ...
    def build_sampling(self, sample: Sample) -> List[StageSampling]:
        ...
        # σ contract self-check: diff_params.sigmas (pinned by the engine for T
        # steps) must have T+1 points. We don't SEND sigmas (BAGEL ignores them),
        # but assert the engine resolved the schedule for the same T the worker
        # will loop.
        ...
        # (observed ratio ≈ 0.8 with the hardcoded 0.99 default). diff_params.sigmas
        # is the engine-pinned T+1-point schedule, identical to the trainside schedule.
...
class BagelOutputAdapter(DitOutputAdapter):
    ...
    def build_segments(self, sample: Sample, per_request: List[List[OmniRawResult]]) -> Dict[str, Any]:
        ...
        return {self.track_name: build_image_segment(diff_outputs, expected_sigmas=expected_sigmas)}
