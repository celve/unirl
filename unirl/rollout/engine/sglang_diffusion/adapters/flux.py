...
text+image→image (SGLang's ``ModelTaskType.TI2I``) — branching on the presence of
``req.primitives['image']``, exactly like the trainside ``Flux2KleinPipeline``.
That is why there is no separate edit adapter key: upstream's pipeline config is
TI2I unconditionally, its ``ImageVAEEncodingStage`` no-ops when no condition image
is set, and ``Flux2KleinConditions`` already declares the image slots optional.
See ``build_prompts`` / ``build_condition`` for the two ti2i deltas.
"""
...
class Flux2KleinAdapter(ImageAdapter):
    ...
    def build_prompts(self, req: RolloutReq) -> Dict[str, Any]:
        ...
        OPTIONAL: absent, this is the inherited T2I payload verbatim. Present,
        the PIL rides the ``condition_image`` sampling kwarg — a SamplingParams
        field injected by :mod:`._patches.patch_sampling_io`, copied onto
        ``Req.condition_image``, and indexed per prompt by that same patch (so a
        multi-prompt batch does not condition every prompt on the last image).
        Upstream's ``InputValidationStage`` then resizes it and
        ``ImageVAEEncodingStage`` VAE-encodes it into ``batch.image_latent`` +
        ``batch.condition_image_latent_ids``. The driver never replicates that
        preprocessing.

        ``stage_config['task'] == 'it2i'`` pins the modality: a dataset that
        carries no ``(image, condition)`` media refs at all then fails here
        instead of silently degrading to T2I and training the wrong objective
        (the reward service would score "is this a good image", not "is this a
        good edit"). Mirrors how ``bagel_editreward.yaml`` pins the same key.
        A batch where only SOME prompts have an image already fails earlier, in
        ``MultimodalRLDataSource``'s ``_validate_homogeneous_images``.
        """
        out = super().build_prompts(req)
        images_prim = req.primitives.get("image")
        if images_prim is None:
            require(
                req.stage_config.get("task") != "it2i",
                f"modality={self.model_family!r}: stage_config.task='it2i' but the request carries no "
                f"primitives['image'] — the dataset is missing (image, condition) media refs. "
                f"Drop the task pin to run text-to-image.",
            )
            return out
        ...
        require(
            len(pil_images) == len(prompts),
            f"build_prompts: image batch {len(pil_images)} != prompt count {len(prompts)}",
        )
        # Collapse the PILs in lockstep with the prompt collapse ``super()`` just
        # did: one source image per group, in first-seen group order. Indexing
        # ``pil_images[::k]`` instead would misalign images vs prompts when
        # ``group_ids`` interleave ([A,B,A,B,...]). ``k == 1`` means no collapse
        # happened (heterogeneous K or no grouping), so the PILs stay 1:1.
        k = int(out.get("num_outputs_per_prompt", 1))
        if k > 1:
            unique_pils = utils.first_per_group(pil_images, list(req.group_ids))
            n_unique_prompts = 1 if isinstance(out["prompt"], str) else len(out["prompt"])
            require(
                len(unique_pils) == n_unique_prompts,
                f"build_prompts: collapsed image count {len(unique_pils)} != unique prompt "
                f"count {n_unique_prompts} (group_ids/image misalignment).",
            )
        else:
            unique_pils = pil_images
        out["condition_image"] = unique_pils if len(unique_pils) > 1 else unique_pils[0]
        return out
    ...
    def build_condition(self, results: List[RawResult]) -> Dict[str, Any]:
        ...
        factorization and FLUX.2 never populates ``vae_image_sizes`` (its
        pipeline config does not override the base ``preprocess_vae_image``
        no-op), so the grid cannot be recovered driver-side without duplicating
        upstream's condition-image resize math.
        """
