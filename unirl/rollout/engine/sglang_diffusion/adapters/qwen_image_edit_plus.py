...
- **Request side.** Edit-Plus **requires** a source-image turn (an ``Images``
  primitive in the sample's conditioning; fail-fast if absent — Edit-Plus is
  edit-only). The adapter extracts PILs
...
class QwenImageEditPlusAdapter(QwenImageAdapter):
    ...
    def build_prompts(self, sample: Sample) -> Dict[str, Any]:
        ...
        Edit-Plus **requires** a source image per prompt — ``vision_conditioning``
        fails loud when the source-image turn is absent (Edit-Plus is edit-only).
        ...
        The K-expanded prompt collapse (``deexpand_prompts_from_groups``) mirrors
        :meth:`ImageAdapter.build_prompts`; we only add the ``condition_image``
        key alongside ``prompt``.
        """
        gen_part = sample.parts[-1]
        # text+image consumer: vision_conditioning() fails loud if the source
        # image turn is absent (Edit-Plus is edit-only) or a non-text/image turn
        # appears. Both primitives are frontier-aligned (one row per gen sample).
        turns, images = sample.vision_conditioning()
        prompts = list(next(t.content for t in turns if isinstance(t.content, Texts)).texts)
