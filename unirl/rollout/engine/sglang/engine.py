...
class SGLangRolloutEngine(BaseSingleTurnRolloutEngine):
    ...
    def continue_generation(
        ...
        *,
        ...
        eos_ids = {int(x) for x in eos} if isinstance(eos, (list, tuple, set)) else ({int(eos)} if eos is not None else set())
        ...
        for row in range(batch_size):
            ...
            while response and int(response[-1]) in eos_ids:
                response.pop()
            full_inputs.append(
                [int(x) for x in real_prompt] + [int(x) for x in response] + prefix_ids
            )
        ...
        require(
            isinstance(frontier.primitive, Texts),
            "SGLang decoder-prefix continuation requires a text response",
        )
        visible = _part_with_field(
            frontier,
            "primitive",
            Texts(texts=[prefix + text for text in frontier.primitive.texts]),
        )
        metadata = []
