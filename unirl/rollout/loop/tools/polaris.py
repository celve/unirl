...
def polaris_headers(provider: str) -> Optional[Dict[str, str]]:
    ...
    if bool(app_id) != bool(app_key):
        raise PolarisAuthError(
            f"{prefix} Polaris authentication requires both app id and app key"
        )
    if not app_id:
        return None
    try:
...
def get_gateway_semaphore(
    capacity: int = 2, gateway: str = POLARIS_GATEWAY_URL
    ...
    with _LIMITER_LOCK:
        ...
        if limiter is None:
            ...
            raise ValueError(
                "all Polaris tools in one process must use the same "
...
class RequestDiagnostics:
    ...
    def as_dict(self) -> Dict[str, Any]:
        return {
            ...
            "status_counts": {
                key: int(value) for key, value in sorted(self.status_counts.items())
            },
        }
