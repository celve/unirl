...
def register_config(
    ...
    *subclass* that adds ``_target_: str = "<target>"`` as a real schema field
    ...
    ``expand`` selects the ``unirl.config.instantiate.build`` dispatch
    ...
    readonly seal applied by ``unirl.config.instantiate.freeze``. A node
    ...
    def _do_register(inner_cls: Type[T]) -> Type[T]:
        ...
        from unirl.config.polymorphic import erase_polymorphic_annotations
