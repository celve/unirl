...
`unirl.sde` owns the per-step diffusion math shared by rollout and
training:
...
That choice lives on `unirl.types.sampling.DiffusionSamplingParams.resolve_sde_indices`
(resolved by the trainer) backed by `unirl.utils.scheduler_utils` (`AllSDEScheduler`,
...
| Concern | Owner |
|---|---|
| Which inference indices run SDE / contribute to training loss | `unirl.types.sampling.DiffusionSamplingParams.resolve_sde_indices(rollout_id)` |
| Window / progressive / random index scheduling (MixGRPO) | `unirl.utils.scheduler_utils.WindowScheduler` |
| Mapping `algorithm.scheduler.*` YAML into a scheduler object | `unirl.utils.scheduler_utils.create_indices_scheduler` |
| Per-model σ override (e.g. video models with custom shift) | `unirl/models/<name>/diffusion.py` |

See `unirl/algorithms/README.md` for the algorithm-side view of this
boundary.
