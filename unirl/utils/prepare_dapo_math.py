...
def _extract_prompt(row: dict, append_boxed: bool) -> str:
    ...
    if isinstance(p, list) and p:  # verl chat format: [{"role","content"}, ...]
        ...
        text = p
    else:  # plain math datasets
        text = row.get("question") or row.get("problem") or ""
    text = (text or "").strip()
...
def _extract_answer(row: dict) -> str | None:
    ...
    for k in ("answer", "solution", "final_answer", "gt"):
        if row.get(k) is not None:
            return str(row[k]).strip()
    return None


def _convert(hf_id: str, split: str, out_path: str, append_boxed: bool) -> int:
...
def main() -> None:
    ...
    _convert(args.dapo_hf, args.dapo_split, os.path.join(args.out_dir, "train.jsonl"), args.append_boxed_template)
