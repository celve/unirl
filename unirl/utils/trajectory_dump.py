    ...
    for metadata in getattr(part, "metadata", None) or []:
        ...
        for diagnostic in diagnostics:
            ...
            for key in _SAFE_TOOL_DIAGNOSTIC_STRINGS:
                ...
                if key == "provider":
                    merged[key] = value if value in _SAFE_TOOL_DIAGNOSTIC_PROVIDERS else "unknown"
...
def _render_trajectory(
    ...
    *,
    ...
    for i, part in enumerate(parts):
        text, truncated = _clip(_turn_text(part.primitives.get("text")), max_chars)
        ...
        answer_rescue_trigger = any(bool((m or {}).get("answer_rescue_trigger")) for m in (part.metadata or []))
        turns.append(
            {
                "index": i,
    ...
    last_gen = _turn_text(gen_parts[-1].primitives.get("text")) if gen_parts else None
    ...
    raw_reward_f = effective_reward_f if raw_reward is None else _finite_or_none(raw_reward)
    record: Dict[str, Any] = {
        ...
        "exclusion_reason": (str(exclusion_reason) if exclusion_reason is not None else None),
        ...
        "num_autonomous_turns": sum(
            ...
            for part in traj.gen_parts()
            if not any(
                bool((m or {}).get("answer_injected")) or bool((m or {}).get("answer_rescued"))
        ...
        "prompt": _turn_text(root.primitives.get("text")) if root is not None else None,
        ...
        with open(path, "w", encoding="utf-8") as fh:
            for i, traj in enumerate(trajs):
                record = _render_trajectory(
                    ...
                    max_chars=max_chars,
                    raw_reward=(raw_rewards_l[i] if i < len(raw_rewards_l) else float("nan")),
                    excluded_from_training=(bool(excluded_l[i]) if i < len(excluded_l) else False),
