...
Kept separate from the config dataclasses (``unirl/train/backend/base.py``) so
the config layer stays torch-free — launcher scripts, linters, and schema tools
can import the typed dataclasses without pulling ``torch``.

The protocols capture the method surface the unirl training code actually uses;
the standard ``torch.optim.AdamW`` / ``torch.optim.lr_scheduler.LRScheduler``
