...
class BagelBundle(Bundle):
    ...
    def from_meta_config(cls, config: BagelPipelineConfig) -> "BagelBundle":
        ...
        * ``_unshard_root``: gather VeOmni's root-sharded leftovers
        ...
        Gen-only: the und ViT tower (``config.enable_vit``) is not supported on the
        meta path — the trainside SP recipe runs ``visual_und=False``.
        """
        from unirl.train.deferred import _stamp

        if config.enable_vit:
            raise NotImplementedError(
                "BagelBundle.from_meta_config: the VeOmni meta-init path is gen-only "
                "(visual_und=False); enable_vit + meta_init_transformer is not supported "
                "(the trainside SP recipe runs gen-only)."
            )
