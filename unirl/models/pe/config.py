...
construction via :func:`unirl.config.instantiate.build`.

Unlike sibling configs (``SD3PipelineConfig`` / ``Qwen3PipelineConfig``)
this one is *not* constructed via the standard
:func:`unirl.config.instantiate.build` path: PE needs DictConfig
...
@register_config(
    group="model",
    name="pe",
    target="unirl.models.pe.PEPipeline.from_config",
    mutable=True,
)
@dataclass
class PEPipelineConfig:
    ...
    classmethod (e.g. ``unirl.models.sd3.SD3Pipeline.from_config``,
    ``unirl.models.qwen3.Qwen3Pipeline.from_config``). Typed as
