...
class DiffusionTrainer(BaseTrainer):
    ...
    def evaluate(self, step: int) -> float:
        ...
        # Each own-set suite then gets its own generation pass over its own prompts.
    ...
    def _eval_pass(
        ...
        bottleneck). Every scorer scores the SAME generated frontier Part —
        single scorable track for now; revisit if multi-track lands.
        """
        ...
        for start in range(0, n_prompts, chunk):
            ...
            for name, scorer in scorers:
                scored = scorer.score_and_attach(sample)
                part = scored.parts[-1]
                if part.rewards is not None:
