...
class UnifiedModelTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        *,
        ...
        # Periodic eval cadence (every eval_interval rollouts; eval_interval=0
        # disables it). Scores only the image track, generated at the
        # deterministic best-quality setting (CFG=eval_cfg_text_scale,
        # eta=eval_eta) — same knobs/semantics as DiffusionTrainer; extra
        # eval-only rewards: unirl.trainer.eval_suites.
        ...
        with placement(self.pool, fraction=1.0, shared_workers=True):
            ...
            self._eval_suites: List[EvalRewardSuite] = build_eval_suites(
    ...
    def _build_request_sample(
        self, inputs: RolloutInputs, rollout_id: int, *, sampling: Optional[Dict[str, BaseSamplingParams]] = None
    ) -> Sample:
        ...
        ``sampling`` overrides the modality-keyed sampling dict — eval forces the
        diffusion sub-block onto the deterministic best-quality setting; the SDE
        schedule is still resolved off whichever diffusion params result. ``None``
        (default) uses ``self.sampling_params`` (the training setting).
        """
        sp = sampling if sampling is not None else self.sampling_params
        diff_params = sp.get("diffusion")
        ar_params = sp.get("ar")
    ...
    def evaluate(self, step: int) -> float:
        ...
        credit-assign/advantage/backward: run the ``P->P*N->P*N*M`` fan-out
        through :meth:`run_rollout` at the deterministic best-quality setting
        (CFG at ``eval_cfg_text_scale``, ``eta=eval_eta``) and score ONLY the
        frontier (image) Part — the training reward plus the ``eval_rewards``
        suites (see :mod:`unirl.trainer.eval_suites`). Logs one ``eval/*`` row;
        returns ``eval/reward``.

        Syncs the live adapter into the engines once per eval (EXTRACT with the
        base onloaded -> wake -> PUSH -> sleep, mirroring :meth:`train_step`'s
        ordering) — train_step syncs BEFORE its generate, so without this the
        engines would eval one update stale. Pushed weights persist across
        sleep/wake, so the passes below just wake/sleep around each chunk's
        rollout. Unlike train_step, eval never onloads the base after the
        extract: there is no backward, so the FSDP state stays offloaded (the
        ...
        # Sync the CURRENT adapter once for the whole eval (PUSH needs awake
        # engines, so it rides one short wake/sleep cycle here). No-op when there
        # is no sync bridge (weight_sync is None).
    ...
    def _eval_pass(
        ...
        Chunked by ``self.batch_size`` (the un-expanded P-prompt request
        DP-splits, so the chunk must be dp-divisible; ``batch_size`` is what
        training runs). A ragged tail (``num_prompts`` not a multiple of
        ``batch_size``) is floored off.
        """
        ...
        for start in range(0, usable, chunk):
            ...
            sample = self._build_request_sample(sub, step, sampling=eval_sp)
            for eng in self.ar_rollouts + self.dit_rollouts:
                eng.wake_up()
            sample = self.run_rollout(sample)
            for eng in self.ar_rollouts + self.dit_rollouts:
                eng.sleep()
            # Score the frontier (image) Part; the reward reads each image's
            # prompt context from the lineage (conditioning), so no manual req
            # expansion is needed.
            for name, reward in scorers:
                scored = reward.score_and_attach(sample)
                part = scored.parts[-1]
                if part.rewards is not None:
                    r = hydrate(part.rewards).to(torch.float32)
