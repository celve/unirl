...
def _infrastructure_group_exclusion(
    ...
    if len(trajs) != len(group_ids):
        raise ValueError(
            f"group_ids must align one-to-one with trajectories; got {len(group_ids)} ids for {len(trajs)} trajectories"
        )
    ...
    for group_id in group_ids:
        ...
        reasons.append("infrastructure_group:" + "+".join(group_reasons) if group_reasons else None)
...
def _tool_diagnostic_metrics(trajs: List[Sample], per_trajectory: List[Dict[str, int]]) -> Dict[str, Any]:
    ...
    """
    totals = {key: sum(int(row.get(key, 0)) for row in per_trajectory) for key in _TOOL_DIAGNOSTIC_ALIASES}
    ...
    metrics: Dict[str, Any] = {
        ...
        "agent/tool_request_trajectory_rate": (
            sum(row.get("request_count", 0) > 0 for row in per_trajectory) / trajectories if trajectories else 0.0
        ),
    }
    metric_names = {
    ...
    metrics["agent/tool_server_error_status_count"] = int(
        sum(count for status, count in statuses.items() if re.search(r"_5[0-9]{2}$", status))
    )
    return metrics


def _trajectory_token_counts(trajs: List[Sample], *, exclude_answer_rescue_triggers: bool = False) -> torch.Tensor:
...
def _prepare_agentic_train_part(part: Part, advantage: float) -> Part:
    ...
    part = _part_with_field(part, "primitives", {})  # free decoded text before train
...
class AgenticTrainer(ARTrainer):
    ...
    def _build_request_sample(
        ...
        *,
        ...
        # ``no_stop_trim`` keeps the matched stop string in decoded text so the
        # conversation and the replayed token stream share the same turn boundary
        # (LIN-564 AReaL parity); omitted entirely when disabled so existing
        # recipes send a byte-identical control bag.
    ...
    def _advantage_train_and_log(
        ...
        *,
        ...
        raw_mean_reward = float(raw_rewards[raw_finite].mean().item()) if bool(raw_finite.any()) else 0.0
        mean_reward = float(effective_rewards[finite].mean().item()) if bool(finite.any()) else 0.0
        ...
        advantages, token_counts = self._compute_agentic_advantages(trajs, effective_rewards, group_ids)
        ...
        log_sample = self._build_log_sample(trajs, effective_rewards, advantages, rollout_id)
        ...
        metrics: Dict[str, Any] = {
            ...
            "reward_std": (
                float(valid_effective_rewards.std(unbiased=False).item()) if valid_effective_rewards.numel() else 0.0
            ),
            "reward_min": (float(valid_effective_rewards.min().item()) if valid_effective_rewards.numel() else 0.0),
            "reward_max": (float(valid_effective_rewards.max().item()) if valid_effective_rewards.numel() else 0.0),
            "agent/mean_turns": (sum(depths) / len(depths)) if depths else 0.0,
            "agent/mean_logical_turns": ((sum(logical_depths) / len(logical_depths)) if logical_depths else 0.0),
            ...
            "agent/answer_injected_rate": (
                sum(1 for count in repair_counts if count > 0) / len(repair_counts) if repair_counts else 0.0
            ),
            "agent/answer_rescued_count": sum(rescue_counts),
            "agent/answer_rescued_rate": (
                sum(1 for count in rescue_counts if count > 0) / len(rescue_counts) if rescue_counts else 0.0
            ),
            ...
            "agent/infra_invalid_group_rate": (invalid_group_count / group_count if group_count else 0.0),
            "agent/infra_invalid_trajectories": invalid_trajectory_count,
            "agent/infra_invalid_trajectory_rate": (invalid_trajectory_count / len(trajs) if trajs else 0.0),
            ...
            "agent/effective_valid_trajectory_rate": (int(finite.sum().item()) / len(trajs) if trajs else 0.0),
            ...
            "agent/max_gen_tokens": (int(generated_token_counts.max().item()) if generated_token_counts.numel() else 0),
        ...
        if bool(token_valid.any()):
            token_weights = token_counts.to(device=effective_rewards.device, dtype=torch.float64)[token_valid]
