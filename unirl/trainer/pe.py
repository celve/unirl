...
class PETrainer(BaseTrainer):
    ...
    def __init__(
        ...
        *,
        ...
        # disables it). Scores only the image ("diffusion") track, generated at
        ...
        # knob: the P->P*N*M two-level fan-out makes a single per-prompt count
        # ambiguous (N*M is already a dense eval).
    ...
    def evaluate(self, step: int) -> float:
        ...
        (CFG at ``eval_cfg_text_scale``, ``eta=eval_eta``), and score the frontier
        (image) Part. The training reward plus every shared-set ``eval_rewards``
        suite scores the SAME generated images; each own-set suite then gets its
        own generation pass over its own prompts. All means land in one ``eval/*``
    ...
    def _eval_pass(
        ...
        P-prompt request, so the chunk must be divisible by the engine dp;
        ``batch_size`` is what training already runs, so it is divisible. A ragged
        ...
        for start in range(0, usable, chunk):
            ...
            sample = self.rollout.generate(sample)
            # Score the frontier (image) Part; the reward reads its prompt context
            # from the Sample lineage (conditioning), so no manual req expansion.
